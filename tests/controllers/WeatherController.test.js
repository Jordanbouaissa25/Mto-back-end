...
//         chai.request(server).post('/weather').query({ city: "Belfort" }).auth(token, { type: "bearer" }).end((err, res) => {
//             // console.log(err, res)
...
//         chai.request(server).post('/weather').auth(token, { type: "bearer" }).end((err, res) => {
//             expect(res).to.have.status(405)
//             done()
//         })
//     })
